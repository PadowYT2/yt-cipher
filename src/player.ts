import Bun from 'bun';
import { playerScriptOverwrites } from '@/metrics';
...
const overridePlayerId = Bun.env.OVERRIDE_PLAYER_ID;
const overridePlayerVariant = Bun.env.OVERRIDE_PLAYER_VARIANT;
...
export function getPlayerScript(playerUrl: string): PlayerScript {
    ...
    if (overridePlayerVariant) {
        ...
        if (variant) {
            playerScriptOverwrites
                .labels({ type: 'variant', source: script.variant, forced: overridePlayerVariant })
                .inc();
